"""
    ...
    Written by: Tianying Yang (tyyang@adobe.com)
...
import sys
sys.path.append(r'D:\eclipse\workspace\client')
...
class childTask(Task):
    ...
    def run(self):
        self.logger.debug('Samba task run')
        ...
        self.getSambaFiles(filePath, sambaUser, sambaPsw, sambaDomain)
    
    def getDownloadList(self):
        return self.__downloadedFileList
    
    def getSingleFile(self, dev, remotePath, ioObj=None):
    ...
    def getSambaFiles(self, url, username, psw, domain):
        self.logger.info('Start downloading from samba location: ' + url)
        ...
        if self.isDir(dev, remotePath):  #folder download
            ...
            self.getSingleFile(dev, remotePath)

if __name__ == '__main__':
    task = childTask('xx', 1)
    task.addPara('sambaDomain', unicode('ADOBENET', 'utf-8'))
    task.addPara('sambaUser', unicode('cndtpqe', 'utf-8'))
    task.addPara('sambaPsw', unicode('CN-qe002', 'utf-8'))
    #task.repository = r"c:\temp"
    task.addPara('Repository', r"c:\temp") 

    task.addPara('FolderPath', unicode(r'blazeman/Builds/Photoshop', 'utf-8'))
    #task.addPara('FolderPath', unicode(r'ps-builds/builds/Photoshop', 'utf-8'))
    task.run()
